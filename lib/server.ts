...
export async function guarded(fn:()=>Promise<Response>){try{return await fn()}catch(e){const code=e instanceof Error?e.message:'';if(['ORIGIN','CONTENT_TYPE','BODY_SIZE'].includes(code))return respond({error:code},400);return respond({error:'SERVICE_UNAVAILABLE'},503)}}
